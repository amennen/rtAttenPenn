...
os.chdir(_thisDir)
configFile=os.path.abspath(os.path.join(os.path.dirname( __file__ ), '../..','config.csv'))
print configFile
...
for r in runs:
    ...
    msgMachine = visual.TextStim(win,text="Waiting for machine...", pos=(0,0),colorSpace='rgb',color=1,height=0.1,wrapWidth=1.5,depth=0.01)
    msgMachine.draw()
    msgMachine.draw()
    win.flip()
    ...
    blocks = data.TrialHandler(nReps=1, method='sequential', 
      ...
      seed=None, name='trials')###CHANGED NAME TO TRIALS
