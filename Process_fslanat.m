...
unix(sprintf('%sepi_reg --epi=%s --t1=%s%s --t1brain=%s%s_brain --out=%s',fslpath,functionalFN_RE,'highres_re.anat/','T1_biascorr','highres_re.anat/','T1_biascorr','example_func2highres_FSLANAT'))
unix(sprintf('%sconvert_xfm -inverse -omat %s.mat %s.mat',fslpath,'highres2exfunc_ANAT','example_func2highres_FSLANAT'));

timefunc2highres = GetSecs-t1;
unix(sprintf('%sconvert_xfm -inverse -omat %s.mat %s.mat',fslpath,'highres2exfunc_ANAT',exfunc2highres_mat));

unix(sprintf('%sinvwarp -w highres_re.anat/T1_to_MNI_nonlin_field.nii.gz -o MNI_to_T1_nonlin -r highres_re.anat/T1_biascorr.nii.gz',fslpath));
...
unix(sprintf('%sapplywarp -i %s/%s.nii.gz -r %s.nii.gz -o %s_exfuncANAT.nii.gz -w highres_re.anat/MNI_to_T1_nonlin_field.nii.gz --postmat=%s.mat',fslpath,roi_dir,roi_name,functionalFN_RE,roi_name,'highres2exfunc_ANAT'));
